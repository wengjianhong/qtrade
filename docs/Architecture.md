...
- **热路径（A 段，§1.3.1）**：行情解析 → **行情通道（Lane-M）** → 策略触发 → CMS → 风控 → OMS 内存更新 → EMS 入队；**发单主链**紧接 **C 段**出站（EMS → 适配器 → 交易所，§1.3.4）。订单/成交回报经 **回报通道（Lane-R）** 进入 OMS 与策略，**优先级高于 Lane-M**（见 §3.1）；**A 段同线程禁止**：同步阻塞、调用支撑服务、等待磁盘 fsync、**等待交易所/远程 I/O 响应**（C 段 RTT 单独计量）。
...
|**交易标准化模块（`TraderNormalizer`）**|跨柜台订单/成交回报语义统一、校验过滤；标准化后交 OMS，再投递 Lane-R|无，引擎固定模块|1. **`TraderNormalizer` 接 `TraderApi` 回报**（规划，见 `trader_normalizer.cpp`）；<br />2. 订单状态、数量单位、错误码映射；<br/>3. **不自有队列**；<br/>4. 出站发单：OMS→EMS→适配器 Api（内部 `OrderRequest` 已统一）|
...
| 通道 | 事件 | 典型路径 | 线程 |
|---|---|---|---|
| **Lane-M** | Tick、Bar（严格入队顺序） | QuoteSpi → QuoteNormalizer → `Publish*` | `MarketEventReactor` 线程 |
| **Lane-R** | Order、Trade（严格入队顺序） | TraderSpi → TraderNormalizer → OMS → `Publish*` | `ReturnEventReactor` 线程 |
...
- **策略并发**：`StrategyEngine` 用 **一把 `mutex_`** 串行 Market/Return 回调（§2.4.3 下实例内同品种仅一策略，通常每 Tick 仅触发一个 `OnTick`）；OMS 自有 `mutex_`。

- **账号/持仓**：OMS 在 Lane-R 处理回报时**同步**更新 AccountManager/PositionManager，不另发总线事件。
- **A 段发单路径**：策略信号 → CMS → Risk → OMS → EMS 入队，以**函数调用**为主（Lane-M 仅驱动策略输入）。**C 段**紧接：EMS 出队 → 执行适配器 → 交易所（§1.3.4）。**B 段**自 OMS 变更并行分叉，不得插入 A→C 之间。
...
| **引擎侧接入** | 由 `QuoteNormalizer` 持有 `QuoteApi` | 由 `TraderNormalizer` 接 `TraderApi` 回报 | 出站 Api 由 EMS 持有；回报经标准化后进 OMS |
...
- **协议适配层 Spi**：厂商协议 → `qtrade_sdk` 类型（Adapter 边界，§7.0）
- **引擎 Lane-M**：`QuoteNormalizer` 语义标准化后 `Publish*` 至 EventBus（§3.1）
- **引擎 Lane-R**：`TraderNormalizer` 语义标准化后交 OMS，再由 OMS `Publish*` 至 EventBus

二者与协议适配器职责不同：适配器解决**外部接口兼容**；标准化模块解决**跨柜台语义统一**；EventBus 解决**排队与热路径隔离**。MVP 中 `QuoteNormalizer` 使用 `SetTickCallback` 与 `RegisterSpi` 并存，逐步统一到 Spi 路径。
...
- **适配器实现层**（§7.0 双向适配）：
    ...
    - **引擎侧**：`TraderNormalizer` 接回报、语义标准化后交 OMS；出站由 EMS 调用 `TraderApi`
    - 企业级扩展：
      - 适配器协议版本兼容，支持交易所协议升级；
      - 适配器流量控制，适配交易所限流规则；
      - 适配器故障重试策略（指数退避）；
...
```Plain
外部行情 → QuoteApi 适配器（Mock/Emt/…）→ QuoteNormalizer → Lane-M
→ StrategyEngine → CMS → RiskManager → OMS（内存）→ EMS 入队
→ EMS 出队 → 执行适配器 → 交易所                    （A 段 │ C 段，发单主链紧连）
     ╲
      → B 段：WAL / 合规日志（异步 I/O，与 OMS 变更并行，不挡 A/C 主链）

订单/成交回报（Lane-R，优先）：
交易所 → 执行适配器 → TraderSpi → TraderNormalizer → OMS 更新 →（同步 Account/PMS）→ Lane-R → StrategyEngine
```
...
```Plain
外部用户/控制台/第三方
  → 外部接入层（HTTP REST，独立项目）
  → QTrade 支撑服务 gRPC（config/history/observability/backtest …）
  ✗ 不直连 qtrade_engine
```
