...
- **热路径（A 段，§1.3.1）**：行情解析 → **行情通道（Lane-M）** → 策略触发 → CMS → 风控 → OMS 内存更新 → EMS 入队；订单/成交回报经 **回报通道（Lane-R）** 进入 OMS 与策略，**优先级高于 Lane-M**（见 §3.1）；**同线程禁止**：同步阻塞、调用支撑服务、等待磁盘 fsync、等待远程 I/O 响应。
...
- **一期（MVP，合并部署）**：
  ...
  - 引擎：**配置驱动多实例分片**（每实例 `engine_id` + **策略及品种绑定**，见 §2.4）+ **`QuoteApi` 适配器**（经 `MarketHandler` 接入，§7.1）
...
| 方式 | 阶段 | 说明 |
|---|---|---|
| **配置驱动静态分片（默认）** | MVP | 部署 N 个实例；每实例通过本地 JSON 或 config-service（按 `engine_id`）获取 `tenant_id`、账户、**策略列表及其绑定品种**；行情 Subscribe 取各策略 `instruments` 的**并集** |
...
**品种 ↔ 策略为多对多**：同一策略可绑定多个品种；同一品种可被多个策略绑定（由 StrategyEngine 按 `strategy_id` 隔离状态与订单）。实例级行情 Subscribe 列表 = 本实例所有已启用策略绑定品种的**并集**（去重）。
...
上例中 `IF2506` 同时绑定两个策略；行情层对 `IF2506` 只 Subscribe 一次，Tick 到达后 StrategyEngine 分发给所有绑定该品种的策略实例。

config-service 通过 `GetConfig(engine_id=engine-03)` 返回该实例专属子集（含策略与绑定）；`WatchConfig` 变更策略启停、参数或品种绑定时，在**维护窗口**重启或 controlled reload，MVP 不做运行时跨实例迁移。
...
|---|---|---|---|
|**事件总线模块**|进程内事件分发，Lane-M / Lane-R 分离|核心基础设施|1. **Lane-R（回报）优先于 Lane-M（行情）**，独立无锁队列；<br />2. Lane-M：Tick/Bar → 策略；Lane-R：Order/Trade → OMS → 策略；<br />3. 控制面变更不进数据通道；|
|**行情处理模块**|接收、清洗、标准化、分发行情数据|`qtrade_sdk::quote::QuoteApi` / `QuoteSpi`（§7.0）|1. **`MarketHandler` 持有 `QuoteApi`**（`SetQuoteApi`，见 `market_handler.cpp`）；<br />2. 行情校验与异常过滤；<br/>3. 本地环形缓冲（规划）；<br/>4. `Subscribe` 本实例策略绑定品种的**并集**；<br/>5. 二期可选 failover（换适配器/重连）与 CPU 绑核|
...
| 通道 | 事件 | 典型路径 | 线程 |
|---|---|---|---|
| **Lane-M** | Tick、Bar（严格入队顺序） | QuoteSpi → MarketHandler → `Publish*` | `MarketEventReactor` 线程 |
| **Lane-R** | Order、Trade（严格入队顺序） | OMS → `Publish*` | `ReturnEventReactor` 线程 |

- **Reactor 语义**：`Publish` 包装为 `TickEvent` / `OrderEvent` 等子类，以 `EventPtr` 入队 → Reactor 线程 `RunOnce()` 出队 → 按 `EventType` 分发至 `TickEventHandler` 等。
- **事件模型**：统一 `Event` 基类 + `EventType` 枚举（见 `event_types.hpp`）；Lane-M / Lane-R 各用独立 Reactor，队列元素类型相同、职责按 Lane 隔离。
- **策略并发**：`StrategyEngine` 用 **一把 `mutex_`** 串行 Market/Return 回调；OMS 自有 `mutex_`。

- **账号 / 持仓**：OMS 在 Lane-R 处理回报时**同步**更新 AccountManager / PositionManager，不另发总线事件。
- **A 段发单路径**：策略信号 → CMS → Risk → OMS → EMS，以**函数调用**为主；Lane-M 仅负责驱动策略输入。
- MVP 已实现 `MarketEventReactor` + `ReturnEventReactor` + `EventLanes`（§12.1）；架构层统称 EventBus，EventReactor 为具体实现。
...
|---|---|---|---|
|**配置中心服务**|集中管理系统配置（数据源参数、**策略及品种绑定**、风控阈值、实例归属等）|支持动态配置更新，配置版本管理，权限控制|1. 配置灰度发布（按租户 / 模块 / **engine_id** 维度）；<br>2. 配置变更审计日志（谁改 / 改了什么 / 何时改）；<br/>3. 配置回滚能力，支持配置生效状态校验；<br/>4. 多租户配置隔离，租户仅可见自身配置；<br/>5. 按 **engine_id** 下发策略列表、品种绑定与账户子集，支撑配置驱动分片（§2.4）|
...
|---|---|---|---|
|**前端控制台**|查询与配置：订单/持仓/绩效查看、监控大盘、策略参数与生命周期配置提交、**实例品种归属**配置|Web 应用（经 API 网关）|1. 多租户视图隔离；<br />2. 按钮级权限（查询 vs 配置提交 vs 审计只读）；<br/>3. 操作日志全记录；<br/>4. 敏感数据脱敏；<br/>5. 多角色（管理员/交易员/审计员）；<br/>6. **不提供「一键下单」类交易操作**|
...
|**多租户管理组件**|租户创建、资源分配、权限配置|集成至 API 网关与配置中心，独立部署|1. 租户生命周期管理（创建 / 暂停 / 注销）；<br />2. 资源配额管理（CPU / 内存 / 行情 / 订单数）；<br/>3. 租户级数据隔离（数据库分库 / 分表）；<br/>4. 租户权限继承与自定义，支持角色模板；<br/>5. 租户与 **engine_id** / 品种归属映射管理（§2.4）|
...
4. **实例品种归属**与行情源配置仅能通过接入层提交至 config-service（按 `engine_id` 下发）
...
- **引擎 Lane-M**：`MarketHandler` 将 Tick/Bar 发布到策略（§3.1）
- **引擎 Lane-R**：TraderSpi 回报经 OMS 更新后通知策略

二者职责不同：前者解决**外部接口兼容**，后者解决**内部模块解耦与热路径隔离**。MVP 中 `MarketHandler` 可同时使用 `SetTickCallback` 与 `RegisterSpi`，逐步统一到 Spi 路径；EventBus 从单队列同步 Publish 迁移至 Lane-M / Lane-R 双队列。

### 7.1 行情数据源（QuoteApi + MarketHandler）
...
行情适配器（QuoteApi 实现，如 MockQuoteApi / EmtQuoteApi）
  → MarketHandler（SetQuoteApi / Subscribe / OnTick → Lane-M）
```

- **`qtrade_sdk::quote::QuoteApi`**：稳定 Target 接口；各厂商 **Api 适配器**实现（§7.0）
- **`MarketHandler`**：引擎侧入口；持有 `std::unique_ptr<QuoteApi> market_source_`；注册 `SetTickCallback` 或 `RegisterSpi`；向 Lane-M 发布 Tick/Bar
- **instruments 过滤**：`MarketHandler::Subscribe` 传入本实例各策略绑定品种的**并集**（去重）；Lane-M 按品种 fan-out 至所有绑定策略（§2.4.3）
- **failover（规划）**：配置中 `quote_source` / `quote_failover` 由引擎在断线时**重建或切换 `QuoteApi` 实例**（或重连同一适配器），逻辑可放在 `MarketHandler` 或启动编排层
...
#### 7.1.5 引擎侧组件（与 `src/engine/market/` 一致）

- **适配器工厂**：如 `CreateMockQuoteApi()`（`src/adapter/mock/quote/`），返回 `std::unique_ptr<QuoteApi>`
- **MarketHandler**：`SetQuoteApi(std::move(api))` 注入适配器；`Subscribe` / `Unsubscribe` 转发至 `QuoteApi`；`OnTick` / `OnBar` → EventBus（目标为 Lane-M）
...
- **策略实现层**：
    ...
    - 策略仅接收本实例 Lane-M / Lane-R 事件；Tick/Bar 按 **strategy_id ↔ instruments** 绑定关系分发（多对多，§2.4.3）
- **管理组件**：
...
- **交易引擎内部**：Lane-M / Lane-R 独立无锁队列（§3.1）；`QuoteApi` 适配器与 `MarketHandler` 间为内存回调（`SetTickCallback` / Spi）
...
```Plain
外部行情 → QuoteApi 适配器（Mock/Emt/…）→ MarketHandler → Lane-M
→ StrategyEngine → CMS → RiskManager → OMS（内存）→ EMS（入队）
→ [B段: WAL 异步] / [C段: 执行适配器 → 交易所]

订单/成交回报（Lane-R，优先）：
交易所 → 执行适配器 → TraderSpi → OMS 更新 →（同步 Account/PMS）→ Lane-R → StrategyEngine
```

配置与控制面数据流：

```Plain
用户 → API 网关 → config-service（校验/审计，含 engine_id、策略及品种绑定）
  ├─ 冷启动: engine ──GetConfig(engine_id)──► config-service（全量）
  └─ 运行时: engine ──WatchConfig（出站订阅）──► 控制线程 → 策略/风控/行情源本地快照
```
...
```Plain
外部用户 → API 网关 → 支撑服务（查询/配置提交/回测/实例负载指标查询）
```
...
- **行情**：每实例 `MarketHandler` + `QuoteApi` 适配器；failover 与跨机房接入点选择分阶段增强（§7.1.3）
...
|阶段|建设重点|交付目标|分片 / 行情|
|---|---|---|---|
|**一期（MVP）**|核心引擎、基础适配器、config + observability 合并服务|单租户实盘（dry-run → 实盘）|**配置驱动多实例** + QuoteApi/MarketHandler（failover 规划）|
...
| 配置驱动分片（engine_id + strategies + 品种绑定） | MVP | 🟡 配置字段待规范；部署文档待补 |
| QuoteApi + MarketHandler | MVP | ✅ 已有（`SetQuoteApi`、Subscribe、PublishTick）；failover 未实现 |
